...
`cjk-layout-audit`는 중국어, 일본어, 한국어 텍스트 레이아웃을 감사하는 agent skill입니다. 대상은 웹페이지, frontend app, 전자책, PDF, 스크린샷, 소스에서 렌더링되는 콘텐츠입니다.

이 skill은 표시되는 CJK 텍스트의 가독성, 줄바꿈, 간격, W3C 텍스트 레이아웃 요구사항과의 정합성을 확인합니다.
...
이 skill을 지원하는 agent runtime에서 다음 prompt를 입력합니다.

```text
$cjk-layout-audit를 사용해 이 CJK 웹페이지 또는 전자책이 W3C 텍스트 레이아웃 요구사항에 맞는지 감사해 주세요.
```

감사 범위가 큰 경우 대상 URL, 로컬 파일, 스크린샷, 전자책 또는 PDF, 예상 언어, 확인 대상 기기 또는 페이지 크기를 지정합니다.
